...
(() => {
  ...
  v4.reservationDetails.unshift(
    {match:'Paris stay',bullets:['予約確認メール上は 9/11 15:00以降チェックイン → 9/17 11:00までにチェックアウト','6泊','予約メール上のゲスト人数は大人1名','リスティング上のゲスト定員は2名','3人全員で同じ宿を利用する予定なら人数条件を要確認','22:00–09:00は騒音禁止の案内あり','正確な住所・確認コード・ホスト連絡先は非公開領域だけで表示']},
  ...
  if (parisStay) {
    ...
    parisStay.meta = 'Bagnolet · 6 nights · 人数要確認';
    parisStay.description = '予約確認メールでは9/11チェックイン、9/17チェックアウト。メール上のゲスト人数は大人1名・定員2名なので、3人利用予定なら条件確認が必要です。';
  }

  const parisCheckin = demo.dayPlans?.['09-11']?.items?.find(item => item.placeId === 'paris-stay');
  if (parisCheckin) parisCheckin.meta = 'Bagnolet · 6 nights · 予約メールの人数条件を要確認';

  const tripStay = (demo.tripItems || []).find(item => item.title === 'Paris stay');
  if (tripStay) {
    tripStay.meta = '11 → 17 Sep · 6 nights · Gmail確認 / 人数条件を要確認';
    tripStay.status = 'attention';
  }

  window.SisterTripReservationTruthV4 = {
    parisStayConflict: '予約メールは大人1名・定員2名。3人全員で同じ宿を利用する予定なら要確認。'
  };
})();
